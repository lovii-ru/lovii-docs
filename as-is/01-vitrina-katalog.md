...
> app d8c3037, минимум заказа энфорсится) и **2026-09-25** (дельта 23–25.09:
> лента акций SZ-075 — раздел ниже). Рабочая заметка (класс W), не канон.
...
- ~~Баннер главной — статичные картинки без API~~ — **заменён 24.09 лентой акций**:
  `PromoFeed` тянет `GET /loyalty/promos` (limit 10, ротация 6 с) и ведёт карточку
  на точку через `promoLink()` — только при `branch_id` от сервиса, иначе ссылка
  честно отсутствует (см. «Дельта 23–25.09»).
...
### Лента акций главной + каталог конструктива (SZ-074/075, staging 24.09)

- **Лента:** публичный `GET /api/v1/loyalty/promos` — только опубликованные точкой
  акции (`storefront.publish`), ротация по часу, сужение `merchant_id`/`branch_id`
  (`ListPromosController.php:14–30`, `routes/api.php:217–221`). Каталог для
  конструктора групп — авторизованный `GET /api/v1/loyalty/catalog`: категории
  и товары точки с ценой (`price` из `merchant_offers`, `ListLoyaltyCatalogQuery.php:29–67`),
  право `merchant.update_profile` (`ListLoyaltyCatalogController.php:15–37`,
  `routes/api.php:312–313`).
- **Клиент:** `PromoFeed` на главной (`HomeModule.vue:51–52`) — скелетон/ошибка/ретрай,
  ротация 6 с (`PromoFeed.vue:44–57`); клик — `router.push(promoLink())`, ссылка
  только при `branch_id` (`promos.ts:10–24`). Экран «Промо» в кабинете МСП —
  маршрут `MspLoyalty` (`router/index.ts:785–786, 833–834`).
- **Числа:** проценты выгоды/скидок — ссылками на канон (`PARAMS.md`); в коде
  только округление подписи (`promos.ts:27–35`).
