...
> Срез: 2026-09-17, **обновлено 2026-09-19** (новый повод: ПЭП-код подписи
> закрывающего документа — SZ-055) и **2026-09-22** (дельта 21.09: per-device
> темы SZ-069, email-канал о заказах закрыт решением владельца). Рабочая заметка
...
- Миграция `push_subscriptions.topics` (jsonb; **NULL = включено всё** — старые
  подписки ничего не теряют, обратная совместимость).
- `PushSubscription::accepts(?string $topic)`: payload без темы (служебные,
  ПЭП-коды) доставляется всегда; `PushPayload::topic` — `order_status`
  (клиентские статусы) / `orders` (новый заказ, напоминания, отмена — точке);
  неизвестная тема = исключение (каталог `PushTopicCatalog.php`, core staging).
- `PushNotificationDispatcher::pushToUserIds` фильтрует подписки по теме —
  клиент получает пуши только на устройства, где тема включена (решение
  владельца 21.09: «строго по настройкам в профиле, на устройства»).
- API: `GET /v1/push/subscriptions` (каталог тем + список своих устройств),
  `PATCH /v1/push/subscriptions/{id}` (topics: null|список; `[]` = выключено
  всё кроме служебных; чужая подписка — 404).
- App: экран управления темами в настройках (SettingsSheet.vue:67–79,
  use-push-notifications.ts); уведомление о новом заказе для точки в app +
  звук нового заказа «как у Яндекса» (решение владельца 21.09).
- Пуши точки включаются у входящего в кабинет + дублёр в канбане b2b
  (SZ-069 виджет, b2b `4538ece`).
...
## Поводы вне заказного контура

- **ПЭП-код подписи** (SZ-055, 18.09): после pull-выплаты НПД создаётся
  закрывающий документ и уходит пуш «Код: N …» — код подписывает ПЭП-отчёт;
  сбой пуша не роняет выплату (есть resend). См. [04](04-bally-koshelek-platezhi.md).
...
**С 21.09 email-канал уведомлений о заказах ЗАКРЫТ решением владельца** —
почта только для отчётов и документов (квитанции, ПЭП). Строки «email» в
таблице выше относятся к срезу до 21.09; новые заказные письма не отправляются.
Клиентские пуши по темам: заказные статусы = тема `order_status` (SZ-069).
...
1. Пуши клиенту на accepted («принят») и ready («можно забирать») — самые
   ожидаемые статусы молчат (см. [03](03-zakazy.md)). → карточка T-009
   (Открыта, 2026-09-18); инфраструктура тем SZ-069 готова — осталось
   добавить поводы в каталог и клиентскую обработку.
2. `push:prune-dead` не в планировщике — мёртвые подписки копятся только при
   отправке; добавить в routes/console.php. → карточка T-009 (там же).
3. Клиентский пуш «заказ создан» — решить, нужен ли (email-канал закрыт 21.09,
   так что пуш — единственный мгновенный повод у клиента).
