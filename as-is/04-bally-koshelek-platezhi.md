...
- Подписка представителя 599/199 — **фундамент реализован 2026-09-18**
  (№2 BACKLOG_REVIEW, SZ-052, core `b21ac96`): таблицы
  `subscriptions` (unique user_id, снимок цены standard|promo, календарный
  период, grace_ends_at) и `subscription_status_changes` (append-only);
  статусы `active / grace_period / suspended / cancelled` — в grace льготы
  СОХРАНЯЮТСЯ (решение владельца §A2); машина переходов — единственная точка
  смены, окно grace 48 ч ставит сама машина (из конфига); конфиг
  `config/subscription.php`: 599/199 ₽, grace 48 ч, retry 6 ч (первая 09:00
  МСК — под №3); read-API `GET /v1/subscription` (без подписки — `data: null`).
  Ещё нет: биллинг/списание (№3 sub-billing: каскад Business → PAY → внешняя
  карта, cron), авто-переходы active→grace→suspended по периоду, гейт доли/
  вывода (№4, PARAMS §1.6), UI; `PAYMENTS_SPLIT_REP_SUBSCRIPTION_GATE` всё
  ещё выключен.
...
4. Подписка 599/199 — фундамент готов (SZ-052, 2026-09-18); осталось: биллинг
   (№3), гейт доли/вывода (№4), UI подписки.
