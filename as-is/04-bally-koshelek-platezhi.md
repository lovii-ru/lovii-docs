...
> по срезу 17.09). Рабочая заметка (класс W), не канон. Числовые параметры —
...
- ⚠️ **Банк пока не исполняет**: до тестового контура Т-Банка реестр уходит в
  интернет-банк **руками** (статус pending; config/payouts.php — тарифы в
  комментариях §C2 #1).
