...
## Чего нет / ограничения (факты, на 25.09)
...
- **Фактические дефолты staging:** минимум 100 ₽, максимум 3 000 ₽,
  неснижаемый остаток PAY 100 ₽; тариф 0,5 / 1,0 / 1,5 / 2,0 / 2,5 / 3,0 /
  3,5% по ступеням (`config/transfers.php:12–29`). Ядро перед каждым quote и
...
- **Расхождение с `PARAMS.md`:** `PARAMS.md:3–7` объявляет единый источник
  истины, однако версия 1.6 не содержит P2P-лимитов и тарифов. Значения выше
  подтверждаются runtime-конфигом и совпадают с `PAY_POINTS_MODEL.md:78–85`,
  но не подтверждены `PARAMS.md`; это явный документационный разрыв, а не
  молчаливое присвоение новых чисел канону.
