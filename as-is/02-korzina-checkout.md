...
> Срез: 2026-09-17, **обновлено 2026-09-19** (минимум заказа энфорсится — №1
> BACKLOG_REVIEW, core 9bf7992 + app e147433). Рабочая заметка (класс W), не
> канон. Формат — [README](README.md).
...
## Минимум заказа (№1 BACKLOG_REVIEW, реализовано 18.09)
...
- остатки: b2b `BranchResource` не тронут — может записать значение ниже пола
  (сработает на расчёте); витринные ресурсы и preview/place отдают
  `min_order_amount_effective[_rubles]` (закрыто финальным мержем `9bf7992`,
  смоук staging 18.09). Правка платформенных 600/500 — страница в супер-админке
  ([10](10-admin.md)).

## Чего нет / ограничения (факты, на 19.09)

- (минимум заказа закрыт 18.09 — см. раздел выше; остаток — b2b-ввод, кандидат ниже)
...
1. ~~Энфорс min_order на чекауте~~ — **сделано 18.09** (см. «Минимум заказа»);
   остаток: b2b-ввод может писать ниже пола — кандидат в задачи b2b.
