...
## Внешний пакет `arena/` — задел на будущее

```
loyalty/arena/ — пакет «Конструктор лояльности» от Arena Agent
был добавлен другим агентом и по решению владельца оставлен как источник
будущих фич.
```

- **Происхождение:** ветка `arena/01a0c9b6-axiiom` репозитория
  `bestdeejay-design/axiiom` (автор Arena Agent) — не наш артефакт, занесён в репо
  механически (см. ниже); в нашёй работе **не участвует**.
- **Что внутри:** 18 механик (LM-01…LM-18), референсный движок `quote/commit`,
  JSON Schema/OpenAPI 3.1/DDL PostgreSQL, 20 примеров кампаний, golden-кейсы.
- **Зачем оставлен:** задел на будущие механики — при развитии промо
  (`promo-roadmap.md`) сверяться с каталогом механик здесь.
- **Статус:** вне области `SZ-072`/`SZ-073`/`SZ-074`/`SZ-075`; не влияет на контракт
  сервиса и дизайн-канон демо.
