...
| Экран | Состояние |
|---|---|
| «Промо» (кабинет МСП) | ✅ **принято, на staging** |
| «Настройки» (новый экран, `/settings`) | 🟡 локально, ждёт приёмки + деплоя |
| Профиль (правки: без настроек, свёрнутые блоки, кнопки «Кошелёк»/«Переводы») | 🟡 локально, ждёт приёмки + деплоя |
